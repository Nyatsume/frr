...
{
  zlog_debug("%s: afi: %d tovpn_sid_index: %d, auto_flag: %d",
       __func__, afi,
				      ...
				      const struct prefix *p, int debug)
{
	 ...
	 */
	if (debug)
		zlog_debug("%s: %pFX srv6_enabled: %d new_attr: l3vpn: %p vpn: %p",
        __func__, p, to_bgp->srv6_enabled, new_attr->srv6_l3vpn,
        new_attr->srv6_vpn);
 ...
 */
static struct bgp_path_info *
path_prefix_sid_update(struct bgp *to_bgp, struct bgp_dest *bn,
	    struct attr *new_attr, /* already interned */
	    afi_t afi, safi_t safi, struct bgp_path_info *source_bpi,
	    mpls_label_t *label, uint32_t num_labels, struct bgp *bgp_orig,
	    struct prefix *nexthop_orig, int nexthop_self_flag, int debug)
{
  ...
  struct bgp *bgp = to_bgp;
	struct srv6_locator_chunk *locator = NULL;
	if (bgp->vpn_policy[afi].tovpn_sid_locator) {
    locator = bgp->vpn_policy[afi].tovpn_sid_locator;
    zlog_debug ("vpn_policy: locator: %p: %pFX.", locator,
                (struct prefix *)&locator->prefix);
	} else if (bgp->tovpn_sid_locator) {
    locator = bgp->tovpn_sid_locator;
    zlog_debug ("tovpn_locator: locator: %p: %pFX.", locator,
                (struct prefix *)&locator->prefix);
  } else {
    zlog_debug ("tovpn_locator: locator: none.");
		ensure_vrf_tovpn_sid(bgp, bgp, afi);
    locator = bgp->vpn_policy[afi].tovpn_sid_locator;
  }

  if (new->extra &&
      (! locator ||
      memcmp (&new->extra->sid[0].sid, locator,
              locator->block_bits_length / 8)))
    {
      if (debug)
        zlog_debug ("%s: extra->sid: %pI6 locator: %pFX",
            __func__, &new->extra->sid[0].sid,
            (struct prefix *)&locator->prefix);
      return NULL;
    }
...
void srv6_prefix_sid_update(struct bgp *bgp,	   /* from */
			      struct bgp_path_info *path_vrf) /* route */
{
	int debug = BGP_DEBUG(vpn, VPN_LEAK_FROM_VRF);
	const struct prefix *p = bgp_dest_get_prefix(path_vrf->net);
	...
	if (debug)
		zlog_debug("%s: from vrf %s", __func__, bgp->name_pretty);
	...
	/* Is this route exportable into the VPN table? */
	if (debug)
		zlog_debug("%s: injectable: %d", __func__,
			is_route_injectable_into_vpn(path_vrf));
	if (!is_route_injectable_into_vpn(path_vrf))
		return;
	...
	/* if policy nexthop not set, use 0 */
  {
	...
	if (bgp->vpn_policy[afi].tovpn_sid_locator) {
		...
		encode_label(
			bgp->vpn_policy[afi].tovpn_sid_transpose_label,
			&label);
		static_attr.srv6_l3vpn = XCALLOC(MTYPE_BGP_SRV6_L3VPN,
				sizeof(struct bgp_attr_srv6_l3vpn));
		static_attr.srv6_l3vpn->sid_flags = 0x00;
		...
		encode_label(bgp->tovpn_sid_transpose_label, &label);
		static_attr.srv6_l3vpn =
			XCALLOC(MTYPE_BGP_SRV6_L3VPN,
				sizeof(struct bgp_attr_srv6_l3vpn));
		static_attr.srv6_l3vpn->sid_flags = 0x00;
		...
		static_attr.srv6_l3vpn->transposition_offset =
			...
			bgp->tovpn_sid_locator->node_bits_length;
		memcpy(&static_attr.srv6_l3vpn->sid,
		       &bgp->tovpn_sid_locator->prefix.prefix,
		       sizeof(struct in6_addr));
	}
	...
	bn = bgp_afi_node_get(bgp->rib[afi][safi], afi, safi, p,
			      &(bgp->vpn_policy[afi].tovpn_rd));

	struct bgp_path_info *new_info;

	new_info =
		path_prefix_sid_update(bgp, bn, new_attr, afi, safi, path_vrf, &label,
			    1, bgp, NULL, nexthop_self_flag, debug);
	 ...
	 */
	if (new_info)
		vpn_leak_to_vrf_update(bgp, new_info, NULL);
	else
#endif
		bgp_dest_unlock_node(bn);
}
...
void srv6_prefix_sid_update_all(struct bgp *bgp, afi_t afi)
{
	struct bgp_dest *bn;
	struct bgp_path_info *bpi;
	int debug = BGP_DEBUG(vpn, VPN_LEAK_FROM_VRF);
	...
	for (bn = bgp_table_top(bgp->rib[afi][SAFI_UNICAST]); bn;
		...
		for (bpi = bgp_dest_get_bgp_path_info(bn); bpi;
			...
			if (bpi->extra) {
				if (debug)
					zlog_debug("%s: skipping path: %p %pBD(type:%d sub:%d)",
						__func__, bpi, bpi->net, bpi->type, bpi->sub_type);
			} else {
#else
			{
#endif
				if (debug)
					zlog_debug("%s: srv6_prefix_sid_update: %p %pBD(type:%d sub:%d)",
						__func__, bpi, bpi->net, bpi->type, bpi->sub_type);
				srv6_prefix_sid_update(bgp, bpi);
			}
		}
	}
}
